...
def animar_caminho(canvas, historico, caminho_final, personagem_img, matriz, objetos_canvas):
    personagem = None
    marcador_fe = []
    poder_fruta = False
    ...
    for passo in historico:
        ...
        for fx, fy, fe in passo['filhos']:
            cx = fy * CELL_SIZE + 10
            cy = fx * CELL_SIZE + 40
            marcador = canvas.create_text(cx, cy, text=f"{fe:.1f}", fill="black", font=("Arial", 8))
            marcador_fe.append(marcador)
        ...
        personagem = canvas.create_image(y * CELL_SIZE, x * CELL_SIZE, anchor=tk.NW, image=personagem_img)
        canvas.update()
        time.sleep(1.0)

    if personagem:
        canvas.delete(personagem)
    personagem = canvas.create_image(caminho_final[0][1] * CELL_SIZE, caminho_final[0][0] * CELL_SIZE, anchor=tk.NW, image=personagem_img)
    canvas.update()
    time.sleep(0.4)
    ...
    for i in range(1, len(caminho_final)):
        ...
        canvas.create_line(
            y0 * CELL_SIZE + CELL_SIZE//2, x0 * CELL_SIZE + CELL_SIZE//2,
        ...
        canvas.delete(personagem)
        personagem = canvas.create_image(y1 * CELL_SIZE, x1 * CELL_SIZE, anchor=tk.NW, image=personagem_img)
...
def iniciar_interface():
    ...
    canvas = tk.Canvas(root, width=len(matriz[0]) * CELL_SIZE, height=len(matriz) * CELL_SIZE)
    canvas.pack()
    ...
    if caminho:
        ...
        personagem_img = ImageTk.PhotoImage(Image.open("img/personagem.png").resize((CELL_SIZE, CELL_SIZE)), master=root)
        root.after(1000, lambda: animar_caminho(canvas, historico, caminho, personagem_img, matriz, objetos_canvas))
    else:
